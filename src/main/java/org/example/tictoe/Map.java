    ...
    private boolean checkWin(int dot){
        for (int i = 0; i < fieldSizeX; i++) {
            for (int j = 0; j < fieldSizeY; j++) {
                if (checkLine(i, j, 1, 0, winLen, dot)) return true;// Горизонтальная проверка
                if (checkLine(i, j, 1, 1, winLen, dot)) return true;// Диагональ (снизу слева вверх справа)
                if (checkLine(i, j, 0, 1, winLen, dot)) return true;// Вертикальная проверка
                if (checkLine(i, j, 1, -1, winLen, dot)) return true;// Диагональ (снизу справа вверх слева)
            }
        }
        return false;
    }
